...
\todo{lieber viele andere Gründe warum es interessant ist..}
...
\todo{Different forms of alumina exist, $\upalpha$, $\beta$ and $\gamma$-Al$_2$O$_3$, but the focus in this work lies on $\upalpha$-alumina.
Also for these different forms, different terminations can be observed.
Their stability differs, (0001) (1\=102) and (11\=20)...}
...
\todo{Reihenfolge entsprechend anpassen!} It starts from the ideas of Hartree-Fock and density functional theory, going to more sophisticated methods like hybrid functionals and perturbation theory, over periodic boundary conditions, to the specialties when dealing with atom centered bases and to \textit{ab initio} molecular dynamics.
Then we will proceed with frequency calculations and intensities needed for the interpretation of vibrational spectra to transition state theory to compute reaction rates, and finally.
...
The approximations used here are based either on wavefunction theory (WFT) or density functional theory (DFT).

\section{Wavefunction Theory}
\subsection{Hartree-Fock Theory}
HF theory is the simplest WFT to solve the electronic Schrödinger equation.
It is based on the approximation of independent particles.
The motion of one electron is considered to be independent from all other electrons; each electrons is only influenced by an averaged field by all other electrons.
...
\end{equation}
and $\langle\alpha|\beta\rangle=\langle\beta|\alpha\rangle=0$ in the bra-ket notation\todo{this is now explained above?}.
...
The simplest ansatz for the total, $N_e$-electron wavefunction would be a product of spin orbitals.
...
This condition is fulfilled in Hartree-Fock theory (HF) by using a single Slater determinant, \textit{i.e.} an antisymmetrized product of spin orbitals, as an ansatz for the ground state (``0''), $N_e$-electron wave function:
\begin{equation}
