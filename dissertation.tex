...
\publishers{\sffamily\large an der\\
  ...
  \includegraphics[width=0.2\textwidth]{./figures/UP-Logo_Matjpg.jpg}\\ \vspace{\baselineskip}
  Potsdam, xx 2018}
\date{}
...
\end{equation}
with the reaction rate constant $k$, Boltzmann's constant $K_B$, the temperature $T$, the Gibb's free energy for the transition state $\Delta G^\ddagger$ and the transmission coefficient $\kappa$, that is a tuneling constant prefactor [tunneling corrections (seldomly applied in this work, not mention them here?)], give the reaction rate constant as a function of temperature and barrier height. To calculate it, one has to find the energy of the transition state and the educt.
To find the transition state geometry and henceforth the energy we used Nudged Elastic Band calculations (NEB), with Climbing Image, Reaction path is approximated as a series of connected images, which are connected via spring forces. These spring forces prevent the images from optimizing into the local minimum next to the transition state. First normal NEB than afterwards climbing image, where the energetically highest image is "optimized" towards higher energies in the contrary direction of the gradient. Check for transition state via frequency analysis, tst of first order has to have one imaginary mode alongside the reaction whose tst it is

\section{From Density Functionals to Hybrids and Perturbation Theory}
In this work we also want to go beyond GGA, because it is known to underestimate reaction rates. Since we are interested in reaction kinetics it therefore is desirable to use more sophisticated methods to improve the rates. First approach is using hybrid functionals, where a fraction of exact exchange is mixed into the potential.
\\
...
\\
For all the vasp calculations for the (0001) surface the parameters from prior work in our workgroup was used, like vacuum gap and convergence criteria, because these were converged carefully by Dr. Jonas Wirth.
\\
