...
\\
To obtain the clean surface slab model a $2\times 2$ supercell was cut from the bulk. The corresponding cell vectors were adopted from the bulk structure and a vacuum gap in z-direction (perpendicular to the surface) was introduced to avoid unphysical interaction between the slabs in z-direction. Since the (11\=20) surface is not the crystal cut of the top of the shown crystal in figure \ref{abb:crystal_11-20}(a), the cell vectors $a$ and $b$ are not equal as in the hexagonal cell, but $a=10.36$\AA, $b=14.16$\AA and $c=20.5$\AA, with the angle between $a$ and $b$ of $\theta=84.56$\textdegree.
\\
...
\small{Reaction Type} & \small{$k_\text{300 K}$(s$^{-1}$)} &  \small{$k_\text{300 K}$(s$^{-1}$)} \\
 ...
 \small{Df-H-c} & still running & still running\\
...
The classical modes calculated with normal mode analysis are dependent on the mass and the spring constant of the bond. This ansatz is quite good for high frequency modes like OD stretch vibrations.
We assumed that the OH/OD stretching modes for the 3 most stable structures (of course for all others as well in order to check whether a structure is a minimum) would contribute to the spectrum the most. Following this approach we expect 6 modes (2 for each, one of the adsorbed OD group and one for the surface OD group). When evaluating those values, for example for the 10 layer slab values in tab.~\ref{tab:freq_layers} one can see that There are three nearly identical vibrations at $2689$, $2692$ and $2694\,$cm$^{-1}$, so that in experiment due to line broadening we expect to see only a single peak. Then there are 3 further peaks, one at $2731\,$cm$^{-1}$ and another one at $2785\,$cm$^{-1}$. The vibrations at $1711\,$cm${-1}$ is out of the experimental range due to its strongly hydrogen bonded nature the wave number is shifted to low $\tilde{\nu}$. Summing up those findings in experiment there should be only 3 bands visible, see figure \ref{abb:exp-sfg}. What we get is very good qualitative agreement with the experimental modes. Quantitatively, the numbers itself do not compare too good, but the relative modes are in really good agreement.
This was done for all slab sizes that were investigated (Table \ref{tab:rel_modes}){\color{red} only do this for example of 10 layers?}:
\begin{table*}[th]
  \centering
 \caption{not absolute but relative frequencies compare very well. All values in cm$^{-1}$.}
\vspace*{.2cm} 
 \begin{tabular}{l|cc}
  layers& $\tilde{\nu}$ &$\delta \tilde{\nu}$\\\hline
  exp. && \\
...
\end{table*}
\\
So first we calculate the Boltzman population to see find out the probability of occupied states:
\begin{equation}\label{boltzmann-weight}
 P_i=exp(-G_i/(k_BT))
\end{equation}
With this analysis it is possible to show the relative population for different temperatures is, see table \ref{tab:boltzmann-pop}.
\begin{table*}[th]
  \centering
...
\end{table*}
This shows, that the probability for the third stable site is so low, that its influence is merely expected in experiment.
\\
For this part, more atomic layers in the slab are considered. As it was mentioned in section \ref{structure_search11-20}, the slab was already converged with 10 layers. In table \ref{tab:freq_layers} the OD frequencies for the vibrations that could be identified in cooperation with experiment for different sizes are shown.
\begin{table*}[th]
  \centering
...
\end{table*}
Comparing the experimental wavenumbers with the computational results one can assign the vibrations in the measurement with the calculated vibrations for both inter-CUSa$\parallel$O-$\mu_2$ vibrations and the adsorbed OD group of CUSb$\parallel$O-$\mu_2$. {\color{red} add chart with delta nu to show this}.
\\
