...
In vacuum, the most stable surface cut of $\upalpha$-Al$_2$O$_3$(0001) is a stochiometric, Al terminated one.
...
\caption{Adsorption geometries for the PBE+D2 optimized geometries of the molecular and the three dissociated species.
Adsorption energies for these species can be found in Table \ref{tab:0001_eads}\todo{die gibts nicht mehr, Daten in die andere Tabelle und dann hier verlinken}.
...
These figures are from unpublished work of Dr. J. Wirth.}
       \label{abb:0001_ads}
\end{figure}
Of these, according to periodic DFT calculations of Reference\cite{WirthJPCC2012} using a plane-wave basis and the PW91 exchange-correlation functional, the 1-2 dissociated species is the most stable one and the 1-4$^\prime$ is the least stable one, see Table \ref{tab:0001_eads} below.
...
Further, the influence of these methods/basis sets on vibrational frequencies will be studied in Section \todo{section with crystal freqs}.
...
To study the influence of the method/basis set on reaction barriers and rates (the latter obtained by Eyring's transition state theory in Section \todo{section crystal rates}, we will exemplarily refer to the hydrogen diffusion from the 1-4 to the 1-2 configuration (called Df-H-4-2 in \cite{WirthJPCC2012} and here).
...
These calculations were conducted by Dr. J. Wirth and are partially unpublished.\todo{später in tabelle mit den crystal rate results integieren! inklusive der dGddagger-Werte}}
\hspace*{-1cm}
 \begin{tabular}{l|cc}
 \toprule
...
\item Finally, we will also test pure wavefunction methods within 2D-slab/AO models,. namely the Hartree-Fock (using CRYSTAL) and local M\o{}ller-Plesset perturbation theory of second order, LMP2 (using cryscor).
\end{enumerate}
...
In fact, as shown in Appendix \ref{app:BSSE}\todo{erstellen!}, for the case of basis 4, where the BSSE was evaluated, about $10\%$ of the adsorption energy is due to it.
...
For the ``customized'' basis set 8, for example, the energy difference between mol and 1-2 diss is $-0.27\,$eV while the PW calculation gives $-0.34\,$eV, and the energy differenc between mol and 1-4 diss is $+0.09\,$eV compared to $-0.01\,$eV.
...
Geometry optimizations were performed and adsorption energies were examined, see Table \ref{tab:basisset-results-B3LYP+D3}.
 \begin{table}[!h]
  \centering
   ...
   The basis sets 4,7 and 8 are used for further calculations and are highlighted in bold.}
  \begin{tabular}{c|ccc}
  \toprule
  basis set & mol & 1-2 diss & 1-4 diss \\\midrule
  1 &-1.90 &-2.25 &-1.83 \\
...
\begin{enumerate}
 \item[(4)] Wavefunction theory with AO basis
\end{enumerate}
...
All values are given in eV. The HF and LMP2 calculations use single point calculations on top of B3LYP optimized geometries with basis set 8.
...
Furthermore, all methods (except for HF as mentioned earlier) agree very well on the stability order.
...
\subsection{Frequency Analysis: Normal Modes and Beyond}
...
\begin{table}[!h]
  ...
  \toprule
   & \multicolumn{2}{c}{PBE+D3/AO} & \multicolumn{2}{c}{B3LYP+D3/AI} &PBE+D2/PW&Exp.\cite{Wirth2014}\\
...
The OD vibrational wavenumbers in the SFG experiments\cite{Wirth2014} there are: $2729$ (1-2: OD$_{\textrm{surf}}$), $2764$ (1-4: OD$_{\textrm{surf}}$), $2790$ (1-4$^\prime$: OD$_{\textrm{surf}}$, was not considerede here) $2900$ (1-4: OD$_{\textrm{ads}}$, 1-4$^\prime$: OD$_{\textrm{ads}}$) and $2910\,$cm$^{-1}$ (1-2: OD$_{\textrm{ads}}$).
...
All values are wavenumbers $\tilde{\nu}$ given in cm$^{-1}$.}
  ...
  \toprule
   resonances& PBE+D3 & PBE+D3+anh. & B3LYP+D3 & B3LYP+D3+anh. &PBE+D2&Exp.\cite{Wirth2014}\\
    & AO & AO & AO & AO & PW & \\\midrule
...
For comparison, previous plane-wave VASP results with PBE+D3 are shown in Table \ref{tab:4-2results_jonas}.
...
\begin{table}[!h]
  ...
  VASP results are given for PBE+D3/PW.}
  \begin{tabular}{c|ccc|c|c}%|cc
  \toprule
    & B3LYP+D3/AO-4 &B3LYP+D3/AO-7 &B3LYP+D3/AO-8&LMP2/AO-9 &PBE+D3/PW\\\midrule
   $\Delta E^\ddagger$ &0.65 &0.65 &0.69 & 0.60&0.44\\
   $\Delta G^\ddagger$ &0.53 &0.54 &0.58 & 0.49$^\ast$ &0.29\\
   $k$(300K) &$8.0\times 10^3$ &$5.8\times 10^3$ &$1.2\times 10^3$ & $3.7\times 10^{4\ast}$& $9.1\times 10^7$\\\bottomrule 
...
Barriers are elevated and rates are lowered with the hybrid functional B3LYP+D3/AO resulting in (free energy) barriers of $\Delta G^\ddagger\sim0.53$-$0.58\,$eV and rates in the order of $10^3$s$^{-1}$, and with LMP2 barriers are slightly lower ($0.49\,$eV), giving higher rates ($10^4$s$^{-1}$) whereas the rate constant with PBE+D3/PW basis is in the order of $10^7$s$^{-1}$. 
...
In all cases a $2\times 2$ supercell was applied with a $3\times 3 \times 1$ $\vec{k}$-point grid.\todo{continue with computational parameters..}
...
To obtain the clean surface slab model of the O-I terminated (11\=20) surface a $2\times 2$ supercell was cut from the bulk.
This termination is the most stable one of five possible terminations and is a stochiometric, oxygen terminated surface\cite{kuri10}.
The corresponding cell vectors were adopted from the bulk structure and a vacuum gap of in z direction (perpendicular to the surface, $17$\AA{} for the 10 layer slab) was introduced to avoid unphysical interaction between the slabs in this direction.
...
The nomenclature and color code of the surface Al and O atoms is also shown: The two types of aluminum atoms are coordinated by the same number of oxygen neighbor atoms but they differ slightly in the arrangement of their neighbors, %and the distance in the relaxed structure,
...
This termination just described is the most stable one and is referred to as O-I termination in reference \cite{kuri10}.
Figure \ref{abb:crystal_11-20}(c) gives the side view and gives the atomic layer sequence O-O$_2$-Al$_4$-O$_2$-O$\ldots$ for the unit cell case and O$_4$-O$_8$-Al$_{16}$-O$_8$-O$_4\ldots$, respectively for the $2\times 2$ cell.
The supercell model that is used mostly in this work has ten atomic layers (two times these layer sequence), with the lowest five fixed to the bulk value to mimic the surface situation.
For the calculations of the lattice vibrations up to 30 layers were considered (Figure \ref{abb:cell_sizes}).
The largest system (30 layers) is only used for one MD calculation due to its high compuational effort, so that the 25 layer system is used instead.
...
\todo{wie viel vakuum für die anderen slab sizes?}}
...
In a next step, water adsorption on the 10 layer relaxed (11\=20) surface model was studied, using PBE+D2, and the same computational parameters as above.
...
\subsection{Other Surface Terminations}
...
We investigated four distinct dissociation reactions: the reactions from CUSb to the twofold coordinated oxygen (D-a) and one to the threefold coordinated O-$\mu_3$ (D-b).
...
\multirow{2}{*}{\small{\ce{H2O} dissociation}} &
 ...
 & \small{D-b} & \textit{\small{0.59}} & \textit{\small{0.53}} & \textit{\small{0.15}} & \textit{\small{0.04}} & \textit{\small{3.0$\times$10$^{13}$}} \\\midrule
...
\begin{figure*} [!ht]
\centering
\subfigure[D-a: CUSb $\rightarrow$CUSb$\parallel$O-$\mu_2$]{\includegraphics[width=0.9\textwidth]{figures/11-20/Diss_Cb-Cb2.pdf}}
         \quad
\subfigure[D-b: CUSb $\rightarrow$CUSb$\parallel$O-$\mu_3$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diss_Cb-Cb3.pdf}}
 \quad
\subfigure[Df-OH-a: CUSb$\parallel$O-$\mu_2$ $\rightarrow$inter-CUSb$\parallel$O-$\mu_2$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-OH_Cb2-iCb2.pdf}}
 \quad
\subfigure[Df-OH-b: CUSb$\parallel$O-$\mu_2$ $\rightarrow$inter-CUSa$\parallel$O-$\mu_2$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-OH_Cb2-iCa2.pdf}}
...
\quad
\subfigure[Df-H-b: inter-CUSa$\parallel$O-$\mu_2\rightarrow$inter-CUSa$\parallel$O-$\mu_3^\prime$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-H_iCa2-iCa3p.pdf}}
 \quad
 ...
 \quad
\subfigure[Df-H-d: inter-CUSb$\parallel$O-$\mu_3^{\prime\prime}$ $\rightarrow$inter-CUSb$\parallel$O-$\mu_3^{\prime\prime\prime}$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-H_iCb3pp-iCb3ppp.png}}
...
Analogous to the AO basis calculations for the (0001) surface in Section \todo{add here the crystal freq section}, the most stable adsorbed species at the (11\=20) surface were optimized with PBE+D3 and B3LYP+D3, respectively and the corresponding vibrational frequencies were evaluated.
...
The abbreviations stand for: iCa2 inter-CUSa$\parallel$O-$\mu_2$, Cb2 CUSb$\parallel$O-$\mu_2$ and iCb2 inter-CUSb$\parallel$O-$\mu_2$.}
  \begin{tabular}{ccc|cc|c|c}
  \toprule
   & \multicolumn{2}{c}{PBE+D3/AO} & \multicolumn{2}{c}{B3LYP+D3/AO} &PBE/PW&Exp.\cite{Heiden11-20_2018}\\
  stretch & $\tilde{\nu}$ [cm$^{-1}$] &$\tilde{\nu}_\textrm{anh}$ [cm$^{-1}$] &$\tilde{\nu}$ [cm$^{-1}$] & $\tilde{\nu}_\textrm{anh}$ [cm$^{-1}$]&$\tilde{\nu}$ [cm$^{-1}$]&$\tilde{\nu}$ [cm$^{-1}$]\\\midrule
...
All values are given in cm$^{-1}$.}
  \begin{tabular}{c|cc|cc|c|c}
  \toprule
   resonances& PBE+D3 & PBE+D3+anh. & B3LYP+D3 & B3LYP+D3+anh.&PBE+D2&Exp.\cite{Heiden11-20_2018}\\
   & AO & AO & AO & AO & PW &\\\midrule
...
These more layered optimized geometries do not differ strongly from the 10 layer ones that were presented in Section \ref{structure_search11-20}, Figure \ref{abb:ads-geoms} and hence are not shown.
...
Especially the 10 layer system is shifted strongly to lower energy compared to the other slab sizes.
...
From $2400$-$2450\,$cm$^{-1}$ in plane vibrations of non hydrogen-bonded species occur and above $2550\,$cm$^{-1}$ out of plane OD stretching bonds appear.
 \begin{figure} [!h]
 \centering
 \subfigure[1ML, lattice range]{\includegraphics[width=0.45\textwidth]{figures/11-20/fully_cov_D2O_slab-sizes-comp_lattice.eps}}
              \quad
             %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. (or a blank line to force the subfigure onto a new line)
  \subfigure[1ML, low OD range]{\includegraphics[width=0.45\textwidth]{figures/11-20/fully_cov_D2O_slab-sizes-comp_OD-low.eps}}
              \quad
             %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. (or a blank line to force the subfigure onto a new line)
...
\begin{figure}[!h]
    ...
    \subfigure[inter-CUSb$\parallel$O-$\mu_2$]{\includegraphics[width=0.5\textwidth]{figures/11-20/comp_bec-dip_iCb2.eps}}
             ...
             All calculations were conducted for the 10 layer system.}
...
As mentioned in section \ref{}, the BSSE was calculted, and corresponding results can be found here.
...
The customized basis set 8 and 9, containing high angular momentum basis functions were not a standard basis sets implemented in CRYSTAL but an individualized one, constructed by Dr. Denis Usvyat, HU Berlin.
