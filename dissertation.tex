...
Density functional theory in the Kohn-Sham scheme for periodic boundary conditions is analog to HF, equations analog to the Roothaan-Hall Equation (\ref{eq:RH-mat}) can be solved with respective periodic Kohn-Sham orbitals. %$\psi_{i\vec{k}}(\vec{r})=\frac{1}{\sqrt{V}}\sum_{\vec{K}} C_{\vec{K}i}e^{(\vec{k}+\vec{K})\vec{r}}$.
