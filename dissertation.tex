...
\uppertitleback{\normalfont
This work has been done between January 2015 and \todo{October} 2018 in the group of Prof. Dr. Peter Saalfrank at the Institute of Chemistry at the University of Potsdam.
}
...
For this, it is reasonable to separate electronic and nucleic degrees of freedom as introduced by the Born-Oppenheimer approximation\cite{bornoppenheimer} (or adiabatic approximation), which is legitimate due to the largely different masses of electrons and nuclei.
...
The Hamiltonian contains the kinetic energy of the electrons, the Coloumb interaction of the electrons and the electron-nucleus interaction:
\begin{equation}
 \hat{H}_e= \frac{1}{2}\nabla^2 - \frac{Z_A}{r_{iA}}
\end{equation}
...
The electrons are described by one electron wavefunctions, that are called (spatial) orbitals $\psi(\vec{r}_i)$.
...
Spin orbitals $\chi(\vec{x})=\psi(\vec{r}_i,\omega)$ contain information about the position and the spin of the electron.
It is also possible to neglect the spin which was done in this work.
...
\begin{equation}
\Psi(x_1,...,x_i,x_j,...x_{N_e})=-\Psi(x_1,...,x_j,x_i,...x_{N_e})
\end{equation}
In other words, two electrons can not have the same set of quantum numbers as is stated in the Pauli principle, which follows from the antisymmetry of the wavefunction: Two electrons must not have the same quantum numbers.

This condition can be easily fulfilled in the Hartree-Fock theory (HF) by a Slater determinant.
\begin{equation}
\Psi^{HF}_{0}({\vec{r_1}},\dots\vec{r_N}) = \frac{1}{\sqrt{N!}}\begin{vmatrix}{\psi_1}({\vec{r_1}})&{\psi_2}({\vec{r_1}})&\dots&{\psi_N}({\vec{r_1}})\\
{\psi_1}({\vec{r_2}})&{\psi_2}({\vec{r_2}})&\dots&{\psi_N}({\vec{r_2}})\\
 \vdots&\vdots&\ddots&\vdots \\
{\psi_1}({\vec{r_N}})&{\psi_2}({\vec{r_N}})&\dots&{\psi_N}({\vec{r_N}})
\end{vmatrix} 
\end{equation}
The choice of a determinant has two advantages: the change of two lines or rows changes the sign and if two lines or rows are the same, the value of the determinant is $0$. Both  are postulated by the Pauli principle.
...
\end{equation}
The exact wavefunction gives the lowest energy, the better the applied wavefunction is, the lower gets the energy.
Minimizing the energy with respect to the choice of the spin orbitals requires the Hartree-Fock equations:
...
% \end{equation}
% The first term is the kinetic energy of the elctron, the second describes the Coulomb interaction between electron $i$ and nucleus $A$ with the charge $Z_A$ and the last term of this equation $v^{HF}(i)$ is an average potential that is experienced by electron $i$ due to the presence of all other electrons.
...
\end{equation}
where $\hat{h}(i)$ is the one-electron Hamiltonian ($\hat{H}_e=\sum_{i=1}^{N_e} \hat{h}(i)$), $\hat{J}$ is the Coulomb operator and $\hat{K}$ is the exchange operator. The latter two are two-electron operators, depending on electrons $i$ and $j$. The sum is only to $N_e/2$ because all spatial orbitals that are doubly occupied are considered.
Furthermore we have
\begin{equation}
 \hat{H}_e=\sum_{i=1}^{N_e}\hat{f}(i)
\end{equation}



The solution of the Hartree-Fock equations gives a set of orthonormal HF-spin orbitals with orbital energies $\varepsilon$.
The orbitals are occupied according to the Aufbau principle (the lowest $N_e$ orbitals), the remaining orbitals are unoccupied, also referred to as virtual. 
...
With a basis (\textit{e.g.} the spatial orbitals $\psi_i$) any arbitrary function $f(\vec{r)}$ can be expaned as a sum:
\begin{equation}\label{eq:basis}
f(\vec{r})=\sum_{i=1}^{N_e}a_i\psi_i(\vec{r})
\end{equation}
The $a_i$ are coefficients.
...
Initially, calculating all energy components from the pure density was not very successful because the exact form of $T[n]$ and $E_{xc}[n]$ is not known and approximations delivered poor results so that instead wave function-based methods were applied commonly.
...
The use of orbitals makes it possible to use the same methods and algorithms previously known from wave function methods\cite{Gross03}.
\begin{equation}\label{eq:KS}
 \left(-\nabla^2 + v_{ext}(\vec{r}) + v_{H}(\vec{r}) + v_{xc}(\vec{r}) \right)\psi^{KS}_i(\vec{r}) = \varepsilon_i\psi^{KS}_i(\vec{r})
\end{equation}
with the kinetic energy, the external potential $v_{ext}(\vec{r})$, the Hartree potential $v_{H}(\vec{r})$, the exchange-correlation potential $v_{xc}(\vec{r})=\frac{\partial E_{xc}[n]}{\partial n(\vec{r})}$, the Kohn-Sham wavefunction $\psi^{KS}_i(\vec{r})$ and the Kohn-Sham orbital energy $\varepsilon_i$.
...
With this, the exchange-correlation functional remains the only unknown functional, but it is typically only a small fraction of the total energy.
...
Formally, only for the exact Kohn-Sham wavefunction the variational principle exists.
% It is a single determinant method and it is not fully variational althoug the ``variational principle of DFT'' does exist.
Also only some expectation values can be calculated from the density, for example the dipole moment as the derivative of the energy with respect to the external electric field. 
% there is no direct calculation of observables possible: not all the expectation values (usually observable=$\langle\Psi|\hat{O}|\Psi\rangle$) can not be calculated via the density.
...
In cases where even higher precision is needed, and correlation (non-classical interaction not included in HF) shall be calculated it is necessary to apply wave function methods.
...
This however, is no eigenfunction of the Hamiltonian but to the one-electron Fock operator.
The reference system $\hat{H}_0$ is perturbed by an external potential $\hat{V}$:
\begin{equation}
 \hat{H} = \hat{H}_0 + \lambda \hat{V}
\end{equation}
$\hat{H}_0$ is the unperturbed Hamiltonian, which is unknown and $\lambda$ is a parameter that determines the extent of the perturbation.
...
This is gained by a localized description of the electron correlation since dynamical correlation is not a long-range effect but can be analyzed locally.
...
In three dimensional systems 14 different form exist, \textit{e.g.} hexagonal D$_\textrm{6h}$.
...
In analogy to the Hartree-Fock equations (Equation (\ref{eq:hf})), we can now introduce the Rootan-Hall expression in a matrix representation, depending on the reciprocal space vector $\vec{k}$:
 \begin{equation}
  \doubleunderline{F}^{\vec{k}}\vec{C}^{\vec{k}}=\doubleunderline{S}^{\vec{k}}\vec{C}^{\vec{k}}\vec{\varepsilon}^{\vec{k}}
 \end{equation}
...
For calculations that depend not only on the density however (like hybrid functionals, MP2), many basis functions are necessary.
...
Lets assume a dissociation reaction: in the molecular case, the electrons can occupy orbitals of both residues (each atom has in total a higher basis set, the binding energy is stabilized), whereas in the dissociated case, this is not possible (the binding energy is not stabilized).
...
\begin{itemize}
 ...
 \begin{equation}
 I\propto |\langle \phi_0|\vec{\mu}|\phi_1 \rangle|^2
 \end{equation}
 $\phi_i$ are eigenfunctions of the harmonic oscillator that are orthonormalized.
 With this, we get
 \begin{equation}
  I\propto |\vec{\mu}_0\langle \phi_0|\phi_1 \rangle + \vec{\mu}^\prime\langle \phi_0|\vec{R}|\phi_1 \rangle|^2
 \end{equation}
 \begin{equation}
  I\propto |0 + \vec{\mu}^\prime\times N|^2
 \end{equation}
 the first term is zero and the second is a constant factor $N$ that is the same for each mode times the change of dipole moment with respect to the coordinates.
...
Only modes with a high perpendicular compound can be measured in respective SFG spectra (sum frequency generation, a surface specific vibrational spectroscopy, see also Appendix \ref{exp_techniques}).
...
\item[II)] Another way to obtain intensities is by applying density functional perturbation theory (DFPT)\cite{Baroni1987,Giannozzi1991}, which is a linear response theory.
Within this approach Born effective charges matrix is determined\cite{Born1954}.
...
These charges can be understood as a coefficient that is related to the change of the polarization due to displacing an atom.
They can be expressed as the derivative of the polarization $P_\beta$ with respect to the coordinates $Z^\star_{\alpha\beta}=\frac{\partial{P_\beta}}{\partial{R_\alpha}}$.
