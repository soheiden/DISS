...
\begin{table}[!h]
  \centering
  \caption{OD stretch vibration results with pob-TZVP/basis set 2 at {\textbf (0001)} surface. Frequencies were calculated at the PBE and B3LYP level of theory with D3 corrections. The presented VASP results were done by J. Wirth with a plane wave and PBE with D3 corrections. ({\color{red}except for mol? Here Jonas only did D2 calculations. Show D2 results for the others instead?}).}
  \begin{tabular}{ccc|cc|c}
   & PBE & & B3LYP & &VASP result\\
  stretch & $\tilde{\nu}$ [cm$^{-1}$] &$\tilde{\nu}_\textrm{anharm}$ [cm$^{-1}$] &$\tilde{\nu}$ [cm$^{-1}$] & $\tilde{\nu}_\textrm{anharm}$ [cm$^{-1}$]& $\tilde{\nu}$ [cm$^{-1}$]\\\hline
...
Analogous to these calculations for the (0001) surface, the (11\=20) surface was optimized with PBE+D3 and B3LYP+D3, respectively and the corresponding vibrational frequencies were evaluated. Corresponding results are shown in Table \ref{tab:freqs_11-20_crystal}.
 \begin{table}[!h]
  \centering
  \caption{Stretch wavenumbers for both of the OD groups at the {\textbf (11\=20)} surface for the 3 most stable species. Frequencies were calculated at the B3LYP and PBE level of theory. The used basis set was basis set 2. The VASP results were obtained with a plane wave basis and PBE+D2 corrections. The abbreviations stand for: iCa2 inter-CUSa$\parallel$O-$\mu_2$, Cb2 CUSb$\parallel$O-$\mu_2$ and iCb2 inter-CUSb$\parallel$O-$\mu_2$.}
  \begin{tabular}{ccc|cc|c}
   & PBE& & B3LYP & &VASP result\\
  stretch & $\tilde{\nu}$ [cm$^{-1}$] &$\tilde{\nu}_\textrm{anharm}$ [cm$^{-1}$] &$\tilde{\nu}$ [cm$^{-1}$] & $\tilde{\nu}_\textrm{anharm}$ [cm$^{-1}$]&$\tilde{\nu}$ [cm$^{-1}$]\\\hline
  iCa2: OD$_{\textrm{surf}}$ &2682 &2599 &2773 &2695 & 2694\\
...
For a selected hydrogen diffusion reaction at the (0001) surface, a transition state with a corresponding rate were recalculated with B3LYP+D3 and basis set 5, since GGA (\textit{e. g.} the functional PBE) are known to underestimate barriers\cite{Zhao05}. This Df-H-4-2 reaction is shown in Figure \ref{abb:df-h-4-2} and was first described in \cite{Wirth2012}.
\\
A transition state optimization was done, but this failed. However, one point of the optimization showed a small energy change in the micro Hartree region, atom displacements were tiny and the gradient was not too large either, so that this structure was examined further. A normal mode analysis showed one single imaginary frequency, as expected for a transition state of first order. Now from this geometry, PBE and MP2 energies were calculated, to calculate a rate with the Eyring equation (\ref{eq:eyring}). These results for B3LYP, PBE and MP2 are shown in Table \todo{Table dE, dG and k(300K) for crystal reoptimized geometry}.
\\
