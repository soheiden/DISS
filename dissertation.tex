...
\todo{trim this section a little?}
...
\\
The 3D unit cell can be described by the three basis vectors of the lattice, $\vec{a}_1$, $\vec{a}_2$ and $\vec{a}_3$ and can be translated along these vectors to gain the infinite system. They span the space with defined length and angles. %\todo{, with seven different models existing.}
...
\end{equation}
According to the Bloch theorem the wave function \todo{$\phi(\vec{r})?$} value at equivalent positions in different cells are related to each other with a phase factor in the lattice vector $\vec{t}$ \todo{sind t und B das gleiche?}:
\begin{equation}
...
\begin{itemize}
 \item[I)] The IR intensities for each normal mode are estimated from the derivative of the dipole moment with respect to the atomic positions, also known as dynamical dipole moment\cite{dyn-dip}. \todo{how does this work in detail? asked Radek for help.} Transition dipole moments of vibrations are determined by $I\propto <i|\hat{\mu}|f>^2$. For this approach, only the dipole components perpendicular to the surface are taken into account. Only these can be measured in respective SFG spectra. These are not completely comparable to IR spectra but IR sensitivity is next to Raman activity one of the selection rules for SFG. Additionally the process is only visible for interfaces (selection rules are shown in Appendix \todo{are they?}).
  
 \item[II)] Another way to get intensities is by calculating Born effective charges, as it was done in the work of Grillo \textit{et al.}\cite{BEC}. \todo{how are these charges calculated? ask Jan!} With the charges the dipole and also the intensities are available through $\mu=\textrm{charge} \times \textrm{distance}$, and again, $I\propto \mu^2$. Both methods are based on normal mode analyses and therefore give IR intensities.
 \end{itemize}
...
\end{equation}
% \todo{was ist N? Ist das Integral wirklich von $-\infty-\infty$ oder sind die Grenzen eingeschränkt? Ist $<x(0)*x(t)>$ ein Mittelwert? Was ist mit der Summe? Im Paper steht folgende Gleichung:
% \begin{equation}
...
VDOS is the vibrational density of states as a function of the frequency $\omega$, summing over all $N$ atoms, Fourier transform of the velocity-velocity autocorrelation function $<v_i(t)\cdot v_i(0)>$. $v_i$ are the velocities at different times $0$ and a later time $t$. The integration range is from $0$ to $\infty$ (in praxis this is just a huge number) due to symmetry considerations. The resulting spectra include implicitly anharmonic and quantum effects: No harmonic approximation for the potential energy surface and the modes are established and movement of the atoms are taken into account. Still, further anharmonic effects like mode coupling, overlapping of neighboring peaks and line broadening are missing\cite{hornicek2007,hudecova2012} \todo{Peter sagt dass das Quatsch ist??}.
The power spectrum contains peaks for each normal mode vector apart from selection rules for vibrational spectroscopy. The obtained intensities correspond to the oscillator amplitudes.
\end{itemize}
...
\end{equation}
with the parameter $a$, that gives the amount of exact Hartree-Fock exchange. As an example, the functional B3LYP consists of $20\%$ exact exchange \todo{citation}.
\begin{equation}
...
\begin{table}[!ht]
  \centering
 \caption{Distances between the top five layers (see also numbering in Figure \ref{abb:crystal_11-20}(c)) for different optimized slab sizes and the unrelaxed bulk structure (right column). All values are given in \AA. \todo{Trennstrich macht Ärger..}} 
\vspace*{.2cm}
...
\subfigure[CUSb+CUSb$\parallel$O-$\mu_2$: OD]{\includegraphics[width=0.45\textwidth]{figures/11-20/comp_Cb+Cb2_OD.eps}}
    ...
    \subfigure[inter-CUSa$\parallel$O-$\mu_2$+CUSb$\parallel$O-$\mu_2$: OD]{\includegraphics[width=0.45\textwidth]{figures/11-20/comp_iCa2+Cb2_OD.eps}}
            \caption{Peaks from normal mode analyses and dipole intensities for two water molecules per supercell (10 layers) in comparison with the corresponding systems with one water molecule, upper line (a,b,c) complete spectrum, lower line (d,e,f) OD range. \todo{umsortieren, dass ein System in einer Zeile ist und entsprechend im Text und caption anpassen!}}
...
\centering
 \includegraphics[width=0.4\textwidth]{figures/theory/sfg-scheme.pdf}
   \caption{Scheme of the SFG Process \todo{blue error tip has wrong color}, left side the spatially overlapping Laser beams are shown, on the right side the orbital representation with the excitation to a virtual orbital is given.}
...
\begin{table}[!h]
 \centering
  \caption{Reaction probabilities for microcanonical (NVE, $T=0$) 
 trajectories of the 2D$_2$O cluster with a clean surface 
 hitting with a kinetic energy $E_\textrm{kin}=0.7$ eV.}
 \begin{tabular}{cccc}
\toprule
