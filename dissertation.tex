...
Also in geochemical sciences Al$_2$O$_3$ is a subject of a variety of studies since aluminum is the third most abundant metal in the crust with  $8.1\%$
...
\ref{abb:2water_comp} in comparison with the respective low coverage analogues.
% \begin{figure}[!h]
%     \centering
...
MP2$^\ast$ {\tiny HF-SCS-MP2} (no dual) & -1.23 & -1.67 & -1.22\\
  \end{tabular}
  \begin{tablenotes}
 \footnotesize
...
\\
A transition state optimization was done, but this failed to converge. However, one point of the optimization showed only a small energy change in the micro Hartree region, atom displacements were tiny and the gradient was not too large either, so that this structure was examined further. A normal mode analysis showed one single imaginary frequency, as expected for a transition state of first order. Now from this geometry, PBE and MP2 energies were calculated, to calculate a rate with the Eyring equation (\ref{eq:eyring}). These results for B3LYP, PBE and MP2 are shown in Table \todo{Table dE, dG and k(300K) for crystal reoptimized geometry}.
\\
Since this obtained transition state geometry was not converged properly, also calculations with PBE, B3LYP and MP2 at the VASP optimized transition state geometry were done and corresponding  outcomes are presented in Table \todo{Table with dE, dG and k(300K) for vasp optimized geometry}.
\\
\todo{
...
\begin{table}[!h]
 ...
 \begin{tabular}{cccc}
\hline
$P_\textrm{mol}$ & P$_\textrm{diss}$(1-2) &  $P_\textrm{diss}$(1-4) & $P_\textrm{refl}$ \\\hline
0.4&0.15+0.05 & 0.05&0.35\\
\hline
\end{tabular}
 \label{tab:2D2O-prob}
\end{table}
