...
Alumina is also applied as a catalyst (for example in the Claus process) and more often as a co-catalyst, for example in the ethylene epoxidation\cite{Oezbek2013} or in the hydrodesulfurization\cite{Parijs1986} where it acts as a base for the active cobalt and molybdenum catalyst in a sulfur removing process:
\begin{equation*}
...
The (11\=20) surface of $\upalpha$-Al$_2$O$_3$ is the third stable one under UHV conditions and was not studied so extensively so far.
...
In pirinciple it can be seen from the data in Table \ref{tab:geom_comp_vasp-crystal}, that both bondlengths and angles with respect to the surface normal are slightly higher with PBE from VASP than with PBE with crystal nd B3LYP, although angles do not differ much.
\begin{table}[!h]
  \centering
...
The first block gives results for VASP/PBE, the second for crystal/PBE and the last one for crystal/B3LYP, both with the high angular momentum basis set.}
...
\end{table}
B3LYP gives the shortest bondlengths and the smallest angles in the comparison but still the results are in very good agreement.
