...
The presented spectra are only preliminary results, since the propagation duration is substantially too short, so that it can not be excluded, that the system is converged well. Usually, for a converged study, a preequilibration phase of several ps is assumed and than the production run is up to 100 or even more ps, but this results still give a good first hint to what the system will look like.
\\
...
\\
\\
Although these results are far from being converged, the agreement with normal mode results is very good and lets hope for even more accurate results with longe propagation duration.
...
\\
(i) Understanding the processes of a water molecule (D$_2$O) being shot at the surface with a molecular beam source with the help of AIMD and
\\
...
\subfigure[(0001), top view]{\includegraphics[width=0.4\textwidth]{figures/0001/surf_0K_axes.pdf}}
 ...
 \caption{Surface model of the (0001), the most stable surface cut under UHV conditions. The top view (a) shows 4 Al CUS atoms, that are surrounded by three threefold coordianted surface oxygen atoms. (b) reveals the Al terminated surface cut in detail with the atomic layers.}
        \label{abb:surf_0001}
\end{figure}
...
\caption{Adsorption geometries of the molecular and the three dissociated species. Adsorption energies for these species can be found in Table \ref{tab:0001_eads+rates}. Additionally, panel (d) shows the numbering of the surface atoms that provides the nomenclature.}
       \label{abb:0001_ads}
\end{figure}
From these, the 1-2 dissociated species is the most stable one and the 1-4$^\prime$ is the least stable one, see left side of Table \ref{tab:0001_eads+rates} \todo{diese Tabelle ist Mist so wie sie jetzt ist..}. For the stability of the molecular and the 1-4 dissociated species, the situation is more controversial in the literature. In our periodic DFT studies with VASP, it lies between the previously mentioned ones, depending on the exact method employed, for PBE+D2 the molecular adsorbed species is more stable than 1-4, for the functional PW91+D2 the 1-4 dissociated is more stable, whereas PW91 without dispersion corrections gives the same adsorption energy for both species (the values for the adsorption energy are calculated analogously to the (11\=20) surface cut, see eq.~\ref{eq:Eads}).
...
\includegraphics[width=0.3\textwidth]{figures/0001/perspective+h2o_new.png}
 \caption{Sketch for inital parameters of the trajectories. The water molecule is situated $4\,$\AA{} above the surface and is arranged within the shown coordinate system, before being impinged to the surface with a defined kinetic energy E$_\textrm{kin}$.}
...
\end{table}
\\
It is now a goal to expand these methods to the following: By first calculating the adsorption energies and the barrier within a atom centered orbital method with the hybrid functional B3LYP and also going beyond density functional theory and going to perturbation theory (LMP2). After this, the transition state shall be reoptimized with B3LYP and the respective rate shall be calculated for this geometry both with density functional theory (PBE and B3LYP) and MP2.
%\\
...
\\
To go beyond density functional theory, first HF and on top of these MP2 calculations were done. The HF calculations were carried out as singlep oint calculations with the optimized geometries from B3LYP calculations with the big basis set. The resulting adsorption energies can be found in Table \ref{tab:Eads_HF}.
\begin{table}[!h]
  \centering
...
\begin{table}[!h]
  \centering
  \caption{OD stretch vibration results with basis set 2 at {\textbf (0001)} surface. Wavenumbers $\tilde{\nu}$ were calculated at the PBE and B3LYP level of theory with D3 corrections and also wavenumbers including anharmonic corrections $\tilde{\nu}_\textrm{anh}$. The presented VASP results were done by J. Wirth with a plane wave and PBE+D2.}
...
In this work great progress was made in understanding the (11\=20) surface of $\upalpha$-Al$_2$O$_3$ in contact with water in the low coverage regime. This surface cut is the third most stable one under UHV conditions and has not been studied to a great extent yet. After optimization of the clean, defect free surface, the stability of different adsorbed species could be classified. One molecular minimum and several dissociated species could be detected, and starting from these reaction rates between minima were evaluated. In addition to reactivity, harmonic vibrational frequencies were determined for comparison with the experimental findings of the experimental group from Fritz-Haber Institute in Berlin. Especially the vibrational frequencies could be assigned with very good agreement to experimental SFG spectra. \todo{Also, lattice vibrations were studied in close collaboration with the experimental partners. As a novelty, they perform SFG spectra at very low frequencies to get deep into the lattice vibration region. Concerning this, a bigger slab model with greater expansion perpendicular to the surface was applied. The overall agreement with lower energy modes is typically worse than for high energy OD(/OH) vibrations.}
\\\\
The dissociation process of water on (0001) surface was studied. This is the most stable surface cut under UHV conditions and was studied by many groups both theoretically and experimentally. In previous work the dissociation was studied by AIMD of molecularly adsorbed water, referring more to an equilibrium situation. The experimental method to obtaining this is pinhole dosing. In contrast to this earlier work, the dissociation process of heavy water when being shot from a molecular beam source was reproduced by periodoc \textit{ab initio} molecular dynamics simulations. This experimental method results in a non-equilibrium situation. This calculations with different surface and beam models let us understand better the results of the non-equilibrium situation. This gives, in contrast to a more equilibrium situation with pinhole dosing, an increase in the dissociation probability, which could be reproduced and also understood mechanistical by those calculations.
\\\\
