...
Direct neighbors do not have a $\prime$, the next neighbor structure is displayed by a single $\prime$, one position further $\prime\prime$ and so on.
...
\begin{equation}
 \text{OH(CUSb)} + \text{H(O-$\mu_2$)} \rightarrow \text{OH(CUSb)} + \text{H(O-$\mu_3$)} \tag{Df-H-a}
     \label{diffHa}
\end{equation}
The reaction is a proton transfer reaction with a barrier height of $0.95\,$eV, but here the NEB transition state has no imaginary frequency so that no rate can be derived, although the NEB path clearly shows a smooth barrier profile, compare Figure \ref{mep2}(a).
...
Additionally, anharmonic corrections for both O-D bonds per system have been performed.
...
Microcanonical (NVE, $T=0$) trajectories of vibrationally and/or rotationally excited D$_2$O approaching a clean surface were simulated with $E_\textrm{kin}=0.7$ eV.}
 \begin{tabular}{cccccc}
\toprule
