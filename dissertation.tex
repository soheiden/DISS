...
For the spin functions we have the Pauli exclusion principle
\begin{equation}
\int_{-\infty}^{\infty}\alpha^\ast(\omega)\alpha(\omega)d\omega=<\alpha|\alpha>=<\beta|\beta>=1
\end{equation}
and $<\alpha|\beta>=<\beta|\alpha>=0$ in the bra-ket notation.
The electrons are then described by one electron wavefunctions, that are called (spatial) orbitals $\psi(\vec{r}_i)$.
...
This condition can be easily fulfilled by a Slater determinant.
\begin{equation}
...
Within the Slater determinant approach, electrons with the same spin are correlated which is called exchange correlation.


With the variational principle the energy of the system can be minimized:
\begin{equation}
E_0=<\Psi^{HF}_0|\hat{H}_e|\Psi^{HF}_0>
\end{equation}
...
\hat{f}(i)\chi(\vec{x}_i)=\epsilon\chi(\vec{x}_i)
\end{equation}
with the orbital energy $\epsilon$ and the Fock operator $\hat{f}(i)$ that is an effective one-electron operator:
\begin{equation}
...
The solution of the Hartree-Fock equations gives a set of orthonormal HF-spin orbitals with orbital energies $\epsilon$.
