...
The method is also known for periodic systems\cite{Suhai1984}\todo{add citation to literature.bib}.
...
This can result in the so-called basis set superposition error (BSSE)\todo{add cite{BSSE}}, which only appears if atom centered bases are utilized.
...
For treating quantum effects such as zero point energy and tunneling it is necessary to solve the nuclear Schrödinger equation, or to use path integral molecular dynamics\todo{cite{PIMD}}.}.
...
\end{equation}
with the eigenvectors $\vec{A}_i$ and the eigenvalues $\lambda_i$ being the squares of vibrational freqencies $\omega_i$: 
