...
In this chapter the basics of the applied theoretical methods shall be explained. {\color{red} MAKE SURE THAT ALL VARIABLES ARE CONSISTENT!}
...
\\
Periodic systems can be described by a unit cell, that is used to generate the infinite system by repeating it. This can be done in 1-D (polymers), 2-D (surface systems) or 3-D (bulk crystals). Also, surfaces can be described with a three dimensional repetition of the unit cell, the only difference is, that one has to define a vacuum gap between two slabs (unit or super cell model of a surface) in one direction (e.g. z). This gap has to be large enough to prevent unit cells from influencing each other and therefore lead to unphysical behaviour between the surface atoms and the lowest atoms of the neighboring slab. Since it is not possible in VASP to mimic a 2-D system, in the main part of this work this vacuum gap 3-D model was applied.Some programs (here used crystal and cp2k(mention it?)) deliver opportunity to calculate only 2D system, repeating the slab only in x/y, a/b, respectively.
\\
The 3-D unit cell can be described by three vectors, $\vec{a}_1$, $\vec{a}_2$ and $\vec{a}_3$ and can be translated along these vectors to gain the infinite system. They span the space with defined length and angles, with seven different models existing, \textit{e.g.} hexagonal crystal family).
