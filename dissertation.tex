...
\addchap{Preamble}
\todo{Umschreiben entlang der Zusammenfassung, Fragestellungen..}
...
The dynamical dipole moment is evaluated by the dipole moment operator $\vec{\hat{\mu}}$ between two states $i$ and $f$, where $\vec{\hat{\mu}}$ is a three component vector.
...
The respective intensities are determined by $I\propto <i|\vec{\hat{\mu}}|f>^2$.
...
\begin{equation}
 VDOS(\omega)=\sum_{i=1}^N\int_{0}^{+\infty}<\vec{v}_i(t)\cdot \vec{v}_i(0)>e^{i\omega t}dt ~.
\end{equation}
...
VDOS is the vibrational density of states as a function of the frequency $\omega$, summing over all $N$ atoms, Fourier transform of the velocity-velocity autocorrelation function $<\vec{v}_i(t)\cdot \vec{v}_i(0)>$.
...
It is apparent from this table that the inter-CUSb||O-$\mu_3^{\prime\prime\prime}$-species is more stable than the species inter-CUSb||O-$\mu_3^{\prime\prime}$ (where the OH groups are closer to each other).
...
This unknown lattice could be a defect side or another surface termination but this suggestion could not be confirmed yet by experimentals data since preparation and probing turns out to be difficult.
...
\begin{figure*} [!ht]
\centering
\subfigure[D-a: CUSb $\rightarrow$CUSb||O-$\mu_2$]{\includegraphics[width=0.9\textwidth]{figures/11-20/Diss_Cb-Cb2.pdf}}
         \quad
\subfigure[D-b: CUSb $\rightarrow$CUSb||O-$\mu_3$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diss_Cb-Cb3.pdf}}
 \quad
\subfigure[Df-OH-a: CUSb||O-$\mu_2$ $\rightarrow$inter-CUSb||O-$\mu_2$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-OH_Cb2-iCb2.pdf}}
 \quad
\subfigure[Df-OH-b: CUSb||O-$\mu_2$ $\rightarrow$inter-CUSa||O-$\mu_2$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-OH_Cb2-iCa2.pdf}}
...
\begin{figure*} [!ht]
\centering
\subfigure[Df-H-a: CUSb||O-$\mu_2\rightarrow$CUSb||O-$\mu_3$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-H_Cb2-Cb3.pdf}}
\quad
\subfigure[Df-H-b: inter-CUSa||O-$\mu_2\rightarrow$inter-CUSa||O-$\mu_3^\prime$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-H_iCa2-iCa3p.pdf}}
 \quad
 \subfigure[Df-H-c: inter-CUSb||O-$\mu_2^\prime\rightarrow$inter-CUSb||O-$\mu_3^{\prime\prime}$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-H_iCb2p-iCb3pp.pdf}}
 \quad
\subfigure[Df-H-d: inter-CUSb||O-$\mu_3^{\prime\prime}$ $\rightarrow$inter-CUSb||O-$\mu_3^{\prime\prime\prime}$]{\includegraphics[width=.9\textwidth]{figures/11-20/Diff-H_iCb3pp-iCb3ppp.png}}
...
Note, that the intensities in the OD range decrease with growing slab size since the normalized to the most intense peak in the lattice region.
For smaller slab sizes the ratio between OD and lattice is higher than for bigger slab sizes, so that the OD intensities in direct comparison seems smaller but has no physical meaning.
...
It was normalized to the most intensity peak.}
\begin{tabular}{cc|cc}
\toprule
...
\begin{figure}[!h]
    ...
    \subfigure[inter-CUSb$\parallel$O-$\mu_2$]{\includegraphics[width=0.5\textwidth]{figures/11-20/comp_bec-dip_iCb2.eps}}
             \caption{Comaprison of IR intensities for the three most stable adsorbed species for intensities from dipole corrections (dip.) and from Born effective charges (BEC). All calculations were conducted for the 10 layer system.}
...
At $300\,$K for the clean surface (30 layers) and the three most stable adsorbed structures (inter-CUSa||O-$\mu_2$, CUSb||O-$\mu_2$ and inter-CUSb||O-$\mu_2$, 10 layers each) the respective AIMD were calculated, starting from the minimum structure with no velocities.
...
We use a $4\times 4 \times 1$ $\vec{k}$-point grid and a \todo{convergence criteria}.
...
In contrast to the VASP calculations employing the plane wave basis, all basis sets that were tested predict the stability order 1-2 $>$ mol $>$ 1-4, whereas VASP depending on the dispersion corrections gives 1-2 $>$ 1-4 $\geqq$ mol. \todo{mention BSSE}
\\
...
The BSSE is positive but by the given definition it must be negative, so that we can conclude that this result is not reliable, maybe due to spin contamination.
...
Optionally, at this point a dual basis expansion can be employed in order to increase the basis even further, so that a basis set of the size of approximately augmented triple zeta can be achieved.
...
However, one point of the optimization showed only a small energy change in the micro Hartree region ($\mu\textrm{E}_h \approx 2.7\times 10^{-5}\,$eV), atom displacements were tiny and the gradient was not too large either, so that this structure was examined further.
