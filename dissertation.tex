...
The structure of $\upalpha$-alumina is well known for several years and was studied in many publications {\color{red} cite here some}. It crystallizes in the hexagonal cell, that means $a=b\neq c$, with an angle of $60$\textdegree~ between $a$ and $b$.
\\
To obtain the clean surface slab model a $2\times 2$ supercell was cut from the bulk. The corresponding cell vectors were adopted from the bulk structure and a vacuum gap in z-direction (perpendicular to the surface) was introduced to avoid unphysical interaction between the slabs in z-direction. {\color{red} Since the (11\=20) surface is not the crystal cut of the top of the shown crystal in figure \ref{abb:crystal_11-20}(a), the cell vectors are not as in the hexagonal cell but $a=10.36$\AA, $b=14.16$\AA and $c=20.5$\AA, with the angle between $a$ and $b$ of $\theta=84.56$\textdegree. CHECK THESE VALUES AGAIN! but should be fine poscar vectors are given like this: a1,a2,a2;b1,b2,b3;c1,c2,c3}
\\
The unit cell consists of 5 atomic layers in z direction (O-O$_2$-Al$_4$-O$_2$-O, see fig.~\ref{abb:crystal_11-20}(c)). The spacing between these layers in the relaxed structure and in the bulk crystal (in parantheses for comparison) are given in \AA:  $d_{12}=0.232$ (0.193) (O-O$_2$ layer distance), $d_{23}=0.642$ (0.741) (O$_2$-Al$_4$), $d_{34}=0.656$ (0.741) (Al$_4$-O$_2$), and $d_{45}=0.198$ (0.191) (O$_2$-O).
\\
The applied supercell has 10, where the lowest 5 were fixed to the bulk value to mimic the surface situation. For the phonon calculations more layers were considered (up to 25 layers){\color{red} see figure xy}, here also for each slab size the lowest 5 layers were kept fixed, respectively (see chapter \ref{phonons}).
\\
{\color{red} Evaluate spacing for the bigger slab sizes and compare}
\begin{figure}[!h]
    \centering
    ...
    \subfigure[(11\=20), side view]{\includegraphics[width=0.30\textwidth]{figures/11-20/uc_opt.png}}
     ...
     \end{figure}
{\color{red} Put here the figures of the different slab sizes! Not much difference for the clean surface with more atomic layers..}
\\
...
{\color{red} Put here stuff with bigger slab size in z direction. Table with Eads for different amount of layers. Explain differences, iCb3 (was it this one?) is slightly different, except from that the ordering is basically the same, the 10 layer cell is already converged with respect to eads and OH frequencies..
}
...
To consider this, geometry optimizations were done where an intact water molecule was planted right above a CUSb and an inter-CUSa position {\color{red} (did I also do MD for this?)}, with the result that one could see in both cases an intermittend molecularly adsorbed structure, that dissociated afterwards to the more stable twofold coordinated oxygen atom.
\\
...
& \small{Df-H-c} & & & &still running & \\
& \small{Df-H-d} & \small{-0.06} & \small{-0.07} & \small{1.05} & \small{0.94} & \small{1.05$\times 10^{-3}$} \\
  \end{tabular}
  \label{tab:reaction-rates}
\end{table*}
\\
It was also tried to study the adsorption/desorption process with NEB but there was no minimum structure in the gas phase nor physisorbed state. The energy profile is smooth from any tested point on the potential energy surface above the alumina surface toward the adsorbed water system without showing a barrier.
\\
As mentioned before a huge problem of GGA is that in this approximation barriers are well known to be underestimated. {\color{red} Optimizing structure with HSE06 is nearly impossible due to high cost and doing single point calculations on PBE optimized transition state is not very accurate and doesn't deliver better results (see also work of Dr. J. Wirth). {\textit (at least for crystal calculations... so don't bring it her?)}}
\\
...
\subfigure[D-a: CUSb $\rightarrow$CUSb||O-$\mu_2$]{\includegraphics[width=0.7\textwidth]{figures/11-20/Diss_Cb-Cb2.pdf}}
         \quad
\subfigure[D-I: CUSb $\rightarrow$CUSb||O-$\mu_3$]{\includegraphics[width=.7\textwidth]{figures/11-20/Diss_Cb-Cb3.pdf}}
 \quad
...
\includegraphics[width=0.9\textwidth]{figures/11-20/SFG_fit.jpg}
 \caption{Experimental SFG results including fit.}
        \label{abb:exp-sfg}
 \end{figure}
Of course, the frequencies for a deuterated system are different from OH, but the different isotope names can be calculated easily within vasp, just by changing the mass. To calculate the modes in this work mainly two methods were applied: Normal mode analyses (see section \ref{nma}) and for some systems also power spectra from \textit{ab-initio} MD via velocity-velocity autocorrelation function (section \ref{vvacf}).
\\
We focus mainly on the position of the peaks rather than the intensities, for the reason of this being not obvious for SFG selection rules (standard programms can do IR or Raman intensities, but not SFG) and the unreliability of those obtained intensities.
In the power spectra the intensities are implicitly received and for the normal mode analyses this work follows 2 different approaches, dipole corrections and Born effective charges.
\\
In subsection \ref{nma}, also modes for higher coverage systems are also examined.
\\
But not only the OH/OD frequencies are of interest, but also the lattice vibrations, see subsection \ref{phonons}.
...
For the phonons as a first test the same normal modes analyses were checked to get a first impression into how the spectra can look like. But for a better description we need to consider more layers of the bulk in order to get more reliable results. From an experimental point of view it is suggested that SFG spectra can give insight into {\color{red} x} layers of the bulk. Therefore we did calculations for the most stable adsorption geometries for more layered systems, going up to 6*5=30 layers (with 5 being the number of layers in the unit cell), both for the clean surface and for the adsorbate covered surface, here we used again optimized geometries of the most stable structures. These results are also shown with intensities calculated from dipole corrected normal mode analyses. The additional geometries with more layers do not differ strongly from the 10 layer ones that were presented in section \ref{structure_search11-20}. {\color{red} Table with no. of layers and adsorption energies? Put it to section \ref{structure_search11-20}? Probably better do do it there!}


\section{Desorption Process}
calculation with NEB didn't lead to anything, there is no barrier visible, check if NEB was done and if not do it!
\begin{verbatim}
...
The (0001) surface is the most stable surface site under UHV conditions, and was subject of several studies so far. Both experimental and theoretical studies discovered lots of characteristics and specialties of this crystal cut. In our workgroup previous work was done concerning the stability of low limit water coverage and their vibrational spectroscopic behaviour, reaction pathways between these stable minima, and also higher water coverages and hydroxylated surface systems. In this work, the focus lies more on two topics:
\\
understanding the dissociation process of a water molecule (D$_2$O) being shot at the surface with the help of \textit{ab-initio} Molecular Dynamics and
\\
the improvement of reaction rates with different methods beyond GGA functionals (as PBE).
\\
First, the surface and the most stable adsorption patterns are introduced (section \ref{0001surf}), followed by the results for the dissociation process and last the improvements for the reaction rates.
	
\section{Surface Model}\label{0001surf}
...
Hass et. al. discussed the idea whether water will adsorb molecularly first and then dissociate or if direct dissociation upon adsorption is possible. To tackle this question for the experimental technique of the molecular beam source, we apply both microcanonical and canonical {\textit ab-initio} MD to simulate the water molecules in the beam approaching the surface. We start in the low coverage limit by letting one single, rigid molecule approach the $2\times 2$ supercell. We later on continue with different approaches to more realistic beams and to more realistic surface situations. In the beam regime we probe water clusters, namely pre-optimized (H$_2$O)$_2$ and (H$_2$O)$_4$ cluster, which are shot onto the surface. Another improvement to the beam lies in having the molecule either vibrationally and/or rotationally excited. These excitations were chosen from a normal mode analysis and the resulting stretch and bending modes.
\\
...
%  \item \bibentry{Wirth12}.
  \item Heiden, S.; Yue, Y.; Kirsch, H.; Wirth, J.; Saalfrank, P.; Campen, R. K.: {\frqq}title{\flqq}, {\textit Journal} {\textbf year,} {\textit vol,} pp.
  \item Heiden, S.;  Saalfrank, P.: {\frqq}title{\flqq}, {\textit Journal} {\textbf year,} {\textit vol,} pp.
