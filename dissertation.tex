...
\\
As a short excursus, also a ``defect site'', the O-II terminated surface, as it is called by Kurita \cite{kuri10} was calculated. As for the O-I termination, 10 layers were considered and from these the lowest five layers were kept fixed to bulk values. Both the clean surface and the fully covered ($1\,$ML) surface were optimized and geometry optimizations were done, in order to have a comparison to the experiment. The structure of the clean surface, of course, differs by the ``missing'' oxygen O-$\mu_2$ atom in the topmost layer, but the fully covered system is more different: of 12 water molecules that can adsorb, 7 are adsorbed molecularly and 5 dissociatively. In comparison, the fully covered system for the O-I termination only shows one molecularly adsorbed molecule.
{\color{red} Figures of structure here and spectra in chapter lattice vibrations!}
