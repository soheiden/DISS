...
\\
(i) understanding the dissociation process of a water molecule (D$_2$O) being shot at the surface with the help of \textit{ab-initio} Molecular Dynamics and
\\
...
{\textit{Crystal structure of $\upalpha$-Al$_2$O$_3$ with surface cuts (0001) and (11\=20) in comparison.}} \\
We applied here a $2\times 2$ supercell with, as before, vectors as in the bulk ({\color{red} or did Jonas optimize them?}) Al terminated surface, only one type of CUS, all oxygen atoms are 3-fold coordinated, topography is not as complicated as for the higher indexed surface. The surface slab that was used consists of 9 atomic layers, that is three repeating units in z direction of the type Al-O$_3$-Al..., with the higher 5 layers being allowed to relax and the lowest 4 layers being fixed.
\\
The stability, vibrations and reactivity of one water  molecule per $2\times 2$ supercell were already studied by Dr. Jonas Wirth. As it was published in ref{jonas0001}, there is one molecular minimum on top of a CUS atom and 3 dissociated states, the next neighboring 1-2 dissociated state, the 1-4 dissociated structure with the Hydrogen atom being one position further and the 1-4$^\prime$ that is the configuration with the greatest distance that is possible for this slab size. From these, the 1-2 dissociated species is the most stable one and the 1-4$^\prime$ is the least stable one. For the molecular and the 1-4 dissociated species, the stability lies between the previously mentioned ones, depending on the method implied, for some methods PW91+D2(?) gives the same adsorption energy (the valus for the adsorption energy are calculated analogously to the (11\=20) surface cut, see eq.~\ref{eq:Eads}), or for some methods and basis sets tested with atom centered orbitals calculations with the  crystal code, the one or the other is more stable.
\\
Of course, there are reactions linking these minima, dissociation, OH- and H-diffusion were studied, as well as rotation of an OH group and molecular water diffusion from CUS to CUS. The latter two do not play a crucial role in this work, especially the CUS to CUS diffusion of molecular water is very improbable due to the slow reaction rate.
\begin{figure}[!ht]
 \centering
