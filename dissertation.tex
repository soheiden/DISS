...
\textit{Problem of plane waves for necessity of atom centered bases}
\\
...
\end{table}
\\
Systems with more layers were \todo{avoid passive?} also studied and the adsorption energy results are shown in Table \ref{tab:eads_layers}. Basically the results do not change much with increasing slab size. The molecular species still is less stable than most of the dissociated species \todo{most? mention the ones thate are different?}. inter-CUSa$\parallel$O-$\mu_2$ is the most stable structure through all the sampled systems, inter-CUSb$\parallel$O-$\mu_2$ and CUSb$\parallel$O-$\mu_2$ change their stability depending on the number of layers but still are some orders of magnitude less probable than inter-CUSa$\parallel$O-$\mu_2$. Dissociated systems occupying the threefold coordinated surface oxygen atom are in the same stability order\todo{remove:}, relatively to each other. For higher number of layers, the inter-CUSb gets more favourable than CUSb, both for O-$\mu_2$ and O-$\mu_3$. This might be due to less interaction between neighboring oxygen atoms with the inter-CUSb adsorbed OH group.
  \begin{table}[!ht]
  \centering
...
\\
Also systems with a higher water coverage were considered\todo{avoid passive}, some cases for 2 water molecules, 4 inter-CUSa$\parallel$O-$\mu_2$ and a fully covered supercell, for these systems also normal mode analyses were done to get vibrational spectra, see also chapter \ref{phonons}.\todo{too many alsos} \todo{Figures of geometries here or later next to the spectrum?}
...
\\
As a short excursus, also a ``defect site'', the O-II terminated surface, as it is called by Kurita \cite{kuri10} was calculated. As for the O-I termination, 10 layers were considered and from these the lowest five layers were kept fixed to bulk values. Both the clean surface and the fully covered ($1\,$ML) surface were optimized and geometry optimizations were done\todo{rephrase. avoid "were done"}, in order to have a comparison to the experiment. The structure of the clean surface, of course, differs by the ``missing'' oxygen O-$\mu_2$ atom in the topmost layer, but the fully covered system is more different: of 12 water molecules that can adsorb, 7 are adsorbed molecularly and 5 dissociatively \todo{chose either active or passive for adsorb}. In comparison, the fully covered system for the O-I termination only shows one molecularly adsorbed molecule.
