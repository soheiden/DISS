...
\\
We investigated 5 different dissociation reactions, 2 of them converged but showed a two step process and are therefore not interesting in the first place ({\color{blue} put them into appendix?}). One additional dissociation starts from the metastable inter-CUSa molecular structure that is, as was shown before, no minimum structure. The two that were already shown in the publication {\color{red} 11-20paper} are the reactions from CUSb to the two fold coordinated oxygen and one to the three fold coordinated O. These dissociation reactions are labeled D-a, D-b and D-c:
 \begin{equation} \label{dissa}
...
\begin{equation}
  \text{H$_2$O(inter-CUSa)} \rightarrow \text{OH(inter-CUSa)} + \text{H(O-$\mu_2$)} \tag{D-c}
      \label{dissc}
\end{equation}
As can be seen from chapter \ref{structure_search11-20} (table \ref{tab:ads_1water}), the molecular minimum CUSb is very low in stability compared to the dissociated minima. The first reaction, D-a leads from the molecular minimum at CUSb to the two-fold coordinated CUSb$\parallel$O-$\mu_2$. The barrier is very low for this dissociation process, see figure \ref{mep}(a). The barrier is very small, about $0.002\,$eV as one can see from table \ref{tab:reaction-rates}, hence the reaction rate constant is very high, in the range of $10^{12}\,$s$^{-1}$. This barrier is very low and might be due to underestimation of barrier heights with the used functional. For the other dissociation reaction D-b from CUSb to CUSb$\parallel$O-$\mu_3$, there is no barrier found at all, the reaction pathway shows simply an upgoing path in energy without any barrier, since the molecular minimum is by $0.15\,$eV more stable than the dissociated structure on CUSb$\parallel$O-$\mu_3$. A further dissociation D-c from the metastable inter-CUSa species also shows no barrier, probably because the educt is no real minimum. {\color{green} optimizations with the intact water molecule placed over CUSb and inter-CUSa lead to a intermittent structure of a molecularly adsorbed species which dissociated later for both cases.}
\\
The OD diffusion reaction Df-OH-a moves from CUSb$\parallel$O-$\mu_2$ to the inter-CUSb position, which is relatively fast for OD diffusion reactions compared to the reactions at other alumina surfaces. In this special reaction, the OH residue diffuses from an on top CUS position into a gap between two CUSb atoms, so that there is not much interaction with neighboring surface oxygen atoms and other CUS atoms nearby. In contrast to that the reaction Df-OH-b   is much slower. It is a diffusion from CUSb to inter-CUSa with the hydrogen residue staying at an O-$\mu_2$ position. This barrier is with $1.07\,$eV higher.
\begin{equation}
...
\begin{table*}[ht]
  \centering
  \caption{{\it Reaction rates for selected reaction paths examined. $\Delta 
